...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<BotProvider>(builder: (context, bot, _) => Scaffold(backgroundColor: AppTheme.darkBg, body: SafeArea(child: CustomScrollView(slivers: [SliverPadding(padding: const EdgeInsets.fromLTRB(18, 18, 18, 32), sliver: SliverList(delegate: SliverChildListDelegate([_header(bot), const SizedBox(height: 18), _accountCard(bot), const SizedBox(height: 10), _accountModeDisplay(bot), const SizedBox(height: 14), _marketCard(bot), const SizedBox(height: 14), _positionCard(bot), const SizedBox(height: 14), _botControl(bot)])))]))));
  }
  ...
  Widget _accountModeDisplay(BotProvider bot) {
    ...
    return _panel(padding: const EdgeInsets.all(12), child: Row(children: [Icon(live ? Icons.shield_rounded : Icons.shield_outlined, size: 18, color: live ? AppTheme.warningColor : AppTheme.secondaryColor), const SizedBox(width: 9), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('ACCOUNT MODE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1)), SizedBox(height: 2), Text('Selected in Settings', style: TextStyle(color: Colors.white38, fontSize: 9))])), _badge(live ? 'LIVE ACCOUNT' : 'DEMO ACCOUNT', live ? AppTheme.warningColor : AppTheme.secondaryColor)]));
  }
