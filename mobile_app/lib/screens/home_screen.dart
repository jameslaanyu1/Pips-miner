...
class HomeScreen extends StatelessWidget {
  ...
  Widget _header(BotProvider bot) {
    ...
    return Row(children: [
      ...
      const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Pips-Miner', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, letterSpacing: 1.0)),
        SizedBox(height: 2),
        Text('life changing pips', style: TextStyle(color: AppTheme.accentColor, fontSize: 9, fontWeight: FontWeight.w700, letterSpacing: .8)),
      ])),
      Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7), decoration: BoxDecoration(color: connected ? AppTheme.successColor.withOpacity(.10) : Colors.white.withOpacity(.05), borderRadius: BorderRadius.circular(20), border: Border.all(color: connected ? AppTheme.successColor.withOpacity(.30) : Colors.white12)), child: Row(children: [
        Container(width: 7, height: 7, decoration: BoxDecoration(shape: BoxShape.circle, color: connected ? AppTheme.successColor : Colors.white30)),
        const SizedBox(width: 6),
        Text(connected ? 'ONLINE' : 'OFFLINE', style: TextStyle(color: connected ? AppTheme.successColor : Colors.white54, fontSize: 9, fontWeight: FontWeight.w800)),
      ])),
    ]);
  }
  ...
  Widget _accountCard(BotProvider bot) {
    return _panel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [const Text('ACCOUNT EQUITY', style: TextStyle(color: Colors.white54, fontSize: 10, fontWeight: FontWeight.w800, letterSpacing: 1.2)), const Spacer(), _badge(bot.accountMode, bot.isLiveAccount ? AppTheme.warningColor : AppTheme.secondaryColor)]),
