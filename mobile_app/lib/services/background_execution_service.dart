...
Future<void> initializePipsMinerBackgroundService() async {
  ...
  const channel = AndroidNotificationChannel(pipsMinerServiceChannel, 'Pips Miner Trading', description: 'Foreground notification for the Pips Miner trading engine.', importance: Importance.low);
  ...
  await service.configure(androidConfiguration: AndroidConfiguration(onStart: pipsMinerBackgroundEntrypoint, autoStart: false, autoStartOnBoot: false, isForegroundMode: true, notificationChannelId: pipsMinerServiceChannel, initialNotificationTitle: 'Pips Miner', initialNotificationContent: 'Trading engine is running', foregroundServiceNotificationId: pipsMinerNotificationId, foregroundServiceTypes: const [AndroidForegroundType.specialUse]), iosConfiguration: IosConfiguration(autoStart: false, onForeground: pipsMinerBackgroundEntrypoint, onBackground: pipsMinerIosBackground));
}

@pragma('vm:entry-point')
Future<bool> pipsMinerIosBackground(ServiceInstance service) async { WidgetsFlutterBinding.ensureInitialized(); DartPluginRegistrant.ensureInitialized(); return true; }
...
Future<void> pipsMinerBackgroundEntrypoint(ServiceInstance service) async {
  ...
  try {
    ...
    if (token == null || token.trim().isEmpty || accountId == null || accountId.trim().isEmpty) throw Exception('No existing Pips-Miner trading session is available. Connect the MT5 account first.');
    ...
    final streamConfig = await api.streamConfiguration();
    final streamToken = streamConfig['token']?.toString().trim() ?? '';
    final streamUrl = streamConfig['streamUrl']?.toString().trim() ?? '';
    if (streamToken.isEmpty || streamUrl.isEmpty) throw Exception('MetaApi live streaming configuration is unavailable.');
    stream = MetaApiStreamingService(token: streamToken, accountId: accountId.trim(), streamUrl: streamUrl);
  } catch (e) {
  ...
  service.on('stopService').listen((_) { stopped = true; tradingTimer?.cancel(); priceSubscription?.cancel(); streamEventSubscription?.cancel(); unawaited(stream.dispose()); engine.stop(); service.stopSelf(); });
  service.on('ping').listen((_) => service.invoke('engineStatus', {'running': engine.running, 'streaming': stream.connected, 'timestamp': DateTime.now().toIso8601String()}));
  ...
  try {
    ...
    priceSubscription = stream.prices.listen((price) { if (!stopped && engine.running) unawaited(engine.tickLive(price)); });
    await stream.connect(config.symbol);
    if (!stream.connected) throw Exception('MetaApi live market stream did not connect.');
  } catch (e) {
    if (MetaApiService.isNetworkFailure(e)) { await emergencyCleanupAndShutdown(); return; }
    service.invoke('engineError', {'fatal': true, 'message': e.toString().replaceFirst('Exception: ', '')});
    engine.stop();
    await stream.dispose();
    service.stopSelf();
  ...
  tradingTimer = Timer.periodic(const Duration(seconds: 1), (timer) async {
    if (stopped || !engine.running) { timer.cancel(); return; }
    try {
      await engine.tick();
      service.invoke('engineStatus', {'running': true, 'streaming': stream.connected, 'timestamp': DateTime.now().toIso8601String(), 'position': engine.activeDirection?.name, 'entryId': engine.activePositionId, 'reversalPrice': engine.reversalPrice});
