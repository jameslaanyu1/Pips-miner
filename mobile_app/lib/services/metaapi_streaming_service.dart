...
class MetaApiStreamingService {
  ...
  Future<void> connect(String symbol) async {
    ...
    final socket = IO.io(
      ...
      IO.OptionBuilder().setTransports(['websocket']).setQuery({'auth-token': token.trim()})
          .disableAutoConnect().enableForceNew().build(),
    );
    _socket = socket;
    ...
    socket.on('synchronization', (raw) {
      ...
      if (type == 'authenticated') {
        ...
        // Market subscriptions are independent of the historical snapshot;
        // request them immediately so ticks are not delayed by synchronization.
        subscribeMarket();
      }
      if (type == 'prices') {
    ...
    socket.onDisconnect((raw) => _events.add({'type': 'disconnected', 'reason': raw?.toString() ?? ''}));

    socket.connect();
    await Future.any<void>([ready.future, Future<void>.delayed(const Duration(seconds: 20))]);
  }
